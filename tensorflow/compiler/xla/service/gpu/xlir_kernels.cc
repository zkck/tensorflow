...
static tfrt::AsyncValueRef<tfrt::Chain> Infeed(
  ...
  return tfrt::EnqueueBlockingWork(
      exec_ctx.host(),
      [=, mutable_stream = stream.ValueRef(),
       mutable_outputs = tfrt::RCArray<tfrt::AsyncValue>(
      ...
      -> llvm::Expected<tfrt::Chain> {
        ...
        auto stream = std::move(mutable_stream);
        auto outputs = std::move(mutable_outputs);
        if (auto error = InfeedBlockingWork(stream, outputs, xla_gpu_params))
          return error;
...
static tfrt::AsyncValueRef<tfrt::Chain> Outfeed(
  ...
  return tfrt::EnqueueBlockingWork(
      exec_ctx.host(),
      [=, mutable_stream = stream.ValueRef(),
       mutable_inputs = tfrt::RCArray<tfrt::AsyncValue>(
      ...
      -> llvm::Expected<tfrt::Chain> {
        ...
        // destructed, which is after returning.
        auto stream = std::move(mutable_stream);
        auto inputs = std::move(mutable_inputs);
        if (auto error = OutfeedBlockingWork(stream, inputs, xla_gpu_params))
          return error;
