...
Status LayoutAssignment::SetBufferLayout(const Layout& layout,
  ...
  VLOG(3) << "Updated constraint:"
          << GetBufferLayoutConstraint(buffer)->ToString() << "\n";
...
Status LayoutAssignment::CalculateComputationLayout(
  ...
  for (HloInstruction* instruction :
    ...
    switch (instruction->opcode()) {
      ...
      case HloOpcode::kWhile:
        if (reverse_computation_order_) {
          // If the loop body doesn't have layouts, propagate existing one
          // inside.
...
Status LayoutAssignment::ConstrainChannelLayouts(
  ...
  for (HloInstruction* instruction : computation->instructions()) {
    if (instruction->opcode() == HloOpcode::kRecvDone) {
      TF_ASSIGN_OR_RETURN(auto operand_layout,
                          InferArrayLayout(instruction, {0}));
      const Layout* layout =
          get_channel_constraints(instruction)
              ->ConstrainChannel(*instruction->channel_id(), operand_layout);
  ...
  // After that we go through the kSend. These are likely going to have a
  // kCopy as operand (otherwise we add it), so in case the constrained layout
  // does not match, we can change the kCopy layout (and the kSend one as
  // well).
  for (HloInstruction* instruction : computation->MakeInstructionPostOrder()) {
    if (instruction->opcode() == HloOpcode::kSend) {
      TF_ASSIGN_OR_RETURN(
          auto operand_layout,
          InferArrayLayout(instruction->mutable_operand(0), {}));
      get_channel_constraints(instruction)
          ->ConstrainChannel(*instruction->channel_id(), operand_layout);
    } else if (instruction->IsCrossModuleAllReduce()) {
      auto* instr_constraints = get_channel_constraints(instruction);
      if (!instr_constraints->IsChannelConstrained(
        ...
        instr_constraints->ConstrainChannel(instruction->channel_id().value(),
                                            op_layout);
      }
    }
  }
